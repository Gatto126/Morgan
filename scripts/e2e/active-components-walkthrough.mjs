...
function isoDate(offsetDays, start = new Date(Date.UTC(2024, 0, 5))) {
...
async function importFile(page, filePath, expectedNewCount, label, openAddDocument = false) {
  if (openAddDocument) {
    ...
    await saveScreenshot(page, `${label}-upload-overlay.png`);
  }
  ...
  await saveScreenshot(page, `${label}-review.png`);
...
try {
  ...
  const cleanupBeforeRun = await cleanupUsersByPrefix(["activeflow"]);
