...
function assertSqliteTarget() {
  const provider = process.env.MORGAN_DATABASE_PROVIDER?.toLowerCase();
  const databaseUrl = process.env.DATABASE_URL ?? "";

  if (provider && provider !== "sqlite") {
    throw new Error("This legacy migration is SQLite-only. Set MORGAN_DATABASE_PROVIDER=sqlite.");
  }

  if (databaseUrl && !databaseUrl.startsWith("file:")) {
    throw new Error("This legacy migration is SQLite-only. DATABASE_URL must use a file: URL.");
  }
}
