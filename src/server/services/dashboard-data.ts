...
export async function getDashboardData(
  ...
  repository: DashboardRepository = dashboardRepository
    ...
    while (current <= end) {
      ...
      if (dayTxs) {
        const dayCheckingTxsByProvider = new Map<string, typeof dayTxs>();

        for (const tx of dayTxs) {
          const provider = getProvider(tx.sourceInstitution);
          
          if (tx.accountType === "checking") {
            const providerDayTxs = dayCheckingTxsByProvider.get(tx.sourceInstitution) ?? [];
            providerDayTxs.push(tx);
            dayCheckingTxsByProvider.set(tx.sourceInstitution, providerDayTxs);
        ...
        for (const [sourceInstitution, providerDayTxs] of dayCheckingTxsByProvider.entries()) {
          const provider = getProvider(sourceInstitution);
          const previousBalance = knownCheckingBalanceProviders.has(sourceInstitution)
            ? provider.checking.total
            : undefined;

          provider.checking.total = resolveDailyEndingBalanceCents(providerDayTxs, previousBalance);
