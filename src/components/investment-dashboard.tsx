...
import { ChartGantt, ChartBar, Wallet, X } from "lucide-react";
...
export function InvestmentDashboard({
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      const response = await fetch(`/api/transactions/investment?userId=${userId}`, { cache: "no-store" });
      const payload = (await response.json()) as InvestmentData & { error?: string };
      if (!response.ok) throw new Error(payload.error ?? "Errore nel caricamento della pagina investimenti.");
  ...
  const fetchLivePrices = useCallback(async (providers: InvestmentProviderSummary[]) => {
    ...
    try {
      const res = await fetch(`/api/prices?isins=${[...allIsins].join(",")}`);
      if (res.ok) {
  ...
  if (loading) return <div className={cn("flex h-full items-center justify-center text-sm text-[color:var(--text-dim)]", !isActive && "absolute inset-0 pointer-events-none opacity-0 invisible")}>Caricamento investimenti...</div>;
            ...
            return (
              ...
              >
                <div className="flex items-center justify-center w-[28px] flex-shrink-0">
                  {tab.key === "ALL" ? (
                    <Wallet className="h-4 w-4 flex-shrink-0" strokeWidth={2.2} />
                  ) : (
                    ...
                    <table className="min-w-full border-separate border-spacing-0 text-[11px] sm:text-sm">
                      ...
                      <tbody>
                        {provider.transactions
                          .filter(tx => {
                            // Strict filtering: Only trades, no crypto
                            const isTrade = tx.tradeType !== null;
                            const desc = (tx.description || "").toLowerCase();
                            const product = (tx.productName || "").toLowerCase();
                            const isCrypto = desc.includes("crypto") || desc.includes("bitcoin") || desc.includes("ethereum") || desc.includes("btc") || desc.includes("eth") ||
                                            product.includes("crypto") || product.includes("bitcoin") || product.includes("ethereum") || product.includes("btc") || product.includes("eth");
