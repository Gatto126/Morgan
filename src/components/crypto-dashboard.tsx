...
import { ChartGantt, ChartBar, Coins, X } from "lucide-react";
...
export function CryptoDashboard({
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      const response = await fetch(`/api/transactions/crypto?userId=${userId}`, { cache: "no-store" });
      const payload = (await response.json()) as InvestmentData & { error?: string };
      if (!response.ok) throw new Error(payload.error ?? "Errore nel caricamento della pagina investimenti crypto.");
  ...
  const fetchLivePrices = useCallback(async (providers: InvestmentProviderSummary[]) => {
    const allSymbols = new Set<string>();
    for (const prov of providers) {
      for (const prod of prov.products) {
        if (prod.isin && prod.quantity > 0.000001) allSymbols.add(prod.isin);
      }
    }
    if (allSymbols.size === 0) return;
    try {
      const res = await fetch(`/api/prices?cryptos=${[...allSymbols].join(",")}`);
      if (res.ok) {
  ...
  // Fetch live prices whenever data changes
  useEffect(() => {
  ...
  if (loading) return <div className={cn("flex h-full items-center justify-center text-sm text-[color:var(--text-dim)]", !isActive && "absolute inset-0 pointer-events-none opacity-0 invisible")}>Caricamento investimenti crypto...</div>;
  ...
  const getProviderLiveTotal = (provider: InvestmentProviderSummary) => {
    let liveTotal = 0;
  ...
  const tabs = [{ key: "ALL", label: "CRYPTO", total: allTotal }, ...data.providers.map(p => ({ key: p.sourceInstitution, label: formatProviderLabel(p.sourceInstitution), total: getProviderLiveTotal(p) }))];
            ...
            return (
              ...
              >
                <div className="flex items-center justify-center w-[28px] flex-shrink-0">
                  {tab.key === "ALL" ? (
                    <Coins className="h-4 w-4 flex-shrink-0" strokeWidth={2.2} />
                  ) : (
                      ...
                      <div key={product.productName} className={isNew ? "card-enter" : undefined}>
                        ...
                        <div key={`product-vals-${product.productName}-${isNew ? "s" : dataVersion}`} className={cn("space-y-1.5 text-sm", !isNew && dataVersion > 0 && "value-flash")}>
                          <div className="flex justify-between">
                            <span className="pl-3 text-[color:var(--text-dim)] font-medium">Token</span>
