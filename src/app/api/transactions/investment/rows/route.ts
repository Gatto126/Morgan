...
const endpoint = "/api/transactions/investment/rows";

export async function GET(request: NextRequest) {
  const trace = createPerformanceTrace("api.endpoint", { endpoint, method: "GET", stage: "investment-rows" });
  ...
  try {
    ...
    const { total, transactions } = await measurePerformanceStep(
      trace,
      "investment.repository.listTransactionRows",
