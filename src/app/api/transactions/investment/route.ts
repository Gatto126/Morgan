...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch historical prices for all unique ISINs
    ...
    while (current <= end) {
      ...
      for (const isin of isins) {
        if (priceMap.get(isin)?.has(curDayKey)) {
          pricesChanged = true;
          break;
        }
      }
      ...
      if (isFirstDay || dayTxs || pricesChanged) {
        ...
        for (const isin of isins) {
          const dayPrice = priceMap.get(isin)?.get(curDayKey);
          if (dayPrice !== undefined) {
            lastKnownPrice.set(isin, dayPrice);
          }
        }
    ...
    for (const [source, products] of productStatus.entries()) {
      ...
      for (const [name, p] of products.entries()) {
        provider.products.push({
          ...
          cashback: 0, // Cashback calculations aren't changed
          isin: p.isin
        });
      }
