...
const endpoint = "/api/transactions/checking/rows";

export async function GET(request: NextRequest) {
  const trace = createPerformanceTrace("api.endpoint", { endpoint, method: "GET", stage: "checking-rows" });
  ...
  try {
    ...
    const { total, transactions } = await measurePerformanceStep(
      trace,
      "checking.repository.listTransactionRows",
