...
- Le sottolinee `BINANCE` nei grafici crypto main e Crypto dashboard seguono la stessa regola: con almeno due punti disegnano una linea; con un solo punto possono disegnare un marker standalone solo se il totale aggregato `CRYPTO`/`heritage` e' nascosto in legenda, altrimenti il marker resta nascosto per non duplicare visivamente il totale.
...
- Un singolo punto Binance corrente/storico non deve essere sommato dentro la linea aggregata `CRYPTO`/`heritage`: resta disponibile come serie `BINANCE` standalone, ma il totale storico lo incorpora solo quando esistono almeno due punti Binance e quindi una linea reale da disegnare.
