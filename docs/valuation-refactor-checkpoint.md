...
- Regola aggregazione punto Binance singolo: nella dashboard principale il punto Binance corrente/storico entra negli aggregati `CRYPTO` e `HERITAGE`. Nella tab `CRYPTO` della main dashboard, il marker standalone `BINANCE` non viene disegnato finche' la linea aggregata `CRYPTO` e' visibile, per evitare doppia rappresentazione dello stesso valore; torna visibile solo se l'utente nasconde l'aggregato e vuole isolare Binance. La Crypto dashboard separata mantiene il comportamento provider dedicato.
